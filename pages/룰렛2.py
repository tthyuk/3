...
st.markdown("룰렛 중심 정렬 및 회전 기능이 완성된 버전입니다!")
...
# --- 룰렛 HTML/CSS 생성 함수 (중심 정렬 로직 수정) ---
def create_roulette_html(numbers, highlighted_number=None, final_pick=False, top_number=None):
    """중심을 정확히 맞춘 룰렛 HTML/CSS를 생성하고 회전시킵니다."""
    
    # --- 룰렛 디자인 변수 ---
    WHEEL_SIZE = 350  # 룰렛 전체 크기 (px)
    NUMBER_DIV_SIZE = 32  # 숫자가 들어갈 원의 크기 (px)
    PADDING_FROM_EDGE = 35 # 룰렛 바깥 테두리와의 여백 (px)
    
    # --- 폰트 크기 자동 조절 ---
    if not numbers: font_size = "16px"
    elif len(numbers) < 18: font_size = "16px"
    else: font_size = "14px"
    ...
    # --- 정확한 중심 계산을 위한 변수 설정 ---
    path_radius = (WHEEL_SIZE / 2) - PADDING_FROM_EDGE
    center_offset = (WHEEL_SIZE / 2) - (NUMBER_DIV_SIZE / 2)
    ...
    for i, num in enumerate(numbers):
        ...
        # ★★★★★ 중심을 맞추기 위해 수정된 계산식 ★★★★★
        x = path_radius * math.cos(angle) + center_offset
        y = path_radius * math.sin(angle) + center_offset
        ...
        items_html += f'<div class="number" style="width: {NUMBER_DIV_SIZE}px; height: {NUMBER_DIV_SIZE}px; top: {y}px; left: {x}px; background-color: {bg_color}; color: {color}; font-weight: {font_weight}; border: {border}; font-size: {font_size};">{num}</div>'
    ...
    <style>
        ...
        .number {{ position: absolute; border-radius: 50%; display: flex; justify-content: center; align-items: center; }}
    </style>
